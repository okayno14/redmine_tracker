...
export_to_csv() ->
    % fun(_) -> ~"x" end
    db:transaction(fun() -> export_to_csv(fun track:to_csv/1) end).
...
export_to_csv(ToCsv) ->
    compose:compose(
        [
            ...
            fun
                (X = #{csv := []}) -> X;
                (X = #{csv := Tracks}) -> X#{timestamp => track:ts_begin(erlang:hd(Tracks))}
            end,
            fun(X) -> X#{csv => tracks:all_sorted_by_timestamp_start()} end
        ],
        #{
            csv => [],
